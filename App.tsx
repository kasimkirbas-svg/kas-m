...
const App = () => {
  ...
  // Auth Handlers (Real simulated login from modal)
  const handleLoginSubmit = (data: any) => {
    // In a real app, you would validate here
    if (data.email === 'admin@kirbas.com') {
      setUser({
        id: '99',
        name: 'System Admin',
      ...
      setUser({
        id: '1',
        name: data.email.split('@')[0] || 'Kullanıcı',
        email: data.email,
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col">
          ...
          <header className="bg-slate-900 text-white py-6 px-4 md:px-12 flex justify-between items-center sticky top-0 z-50">
             ...
             <div className="space-x-4">
               <button onClick={() => setIsAuthModalOpen(true)} className="text-slate-300 hover:text-white font-medium">Giriş Yap</button>
               <Button onClick={() => setIsAuthModalOpen(true)} variant="primary">Kayıt Ol</Button>
             </div>
          </header>
          ...
          <section className="bg-slate-900 text-white py-20 px-6 text-center">
            ...
            <div className="flex justify-center gap-4">
              <Button onClick={() => setIsAuthModalOpen(true)} size="lg">Hemen Başla</Button>
            </div>
          </section>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {PLANS.map(plan => (
                <div key={plan.id} className={`bg-white p-8 rounded-2xl shadow-sm border ${plan.popular ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200'}`}>
                  ...
                  <Button onClick={() => setIsAuthModalOpen(true)} className="w-full mt-8" variant={plan.popular ? 'primary' : 'outline'}>
                    Seç ve Başla
                  </Button>
                </div>
              ))}
          ...
          {/* Auth Modal overlay for the landing page */}
          <AuthModal 
