...
export const Billing: React.FC<BillingProps> = ({ user, onSelectPlan }) => {
  ...
  return <section className="max-w-6xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
    ...
    <div className="grid md:grid-cols-2 gap-5">
      {plans.map(plan => <button key={plan.id} onClick={() => { setSelected(plan.id); setSuccess(false); }} className={`relative text-left p-6 sm:p-8 rounded-xl border transition-all backdrop-blur-sm ${selected === plan.id ? 'border-amber-400 ring-2 ring-amber-400/20 bg-yellow-50 dark:bg-amber-400/10' : 'border-slate-200 dark:border-white/10 bg-white/80 dark:bg-[#171b20]/92 hover:border-amber-400/50'}`}>
