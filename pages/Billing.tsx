...
export const Billing: React.FC<BillingProps> = ({ user, onSelectPlan }) => {
  ...
  return <section className="max-w-6xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
    <div className="max-w-2xl mb-9"><div className="flex items-center gap-2 text-yellow-600 dark:text-yellow-400 text-xs font-black tracking-widest uppercase"><Crown size={16} /> Üyelik Merkezi</div><h1 className="mt-3 text-3xl sm:text-4xl font-black text-slate-950 dark:text-white">İhtiyacınıza uygun planı seçin</h1><p className="mt-3 text-slate-600 dark:text-slate-400">Plan değişikliği hesabınıza anında uygulanır. Güvenli ödeme sağlayıcısı bağlandığında kart tahsilatı bu ekrandan tamamlanacaktır.</p></div>
    {success && <div className="mb-6 p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 flex items-center gap-3"><ShieldCheck size={20} /> Plan tercihiniz hesabınıza kaydedildi.</div>}
    <div className="grid md:grid-cols-2 gap-5">
      {plans.map(plan => <button key={plan.id} onClick={() => { setSelected(plan.id); setSuccess(false); }} className={`relative text-left p-6 sm:p-8 rounded-xl border transition-all backdrop-blur-sm ${selected === plan.id ? 'border-yellow-500 ring-2 ring-yellow-500/20 bg-yellow-50 dark:bg-yellow-500/10' : 'border-slate-200 dark:border-white/10 bg-white/80 dark:bg-[#242821]/88 hover:border-yellow-500/50'}`}>
    ...
    <button onClick={activate} disabled={selected === 'FREE'} className="mt-6 w-full sm:w-auto min-h-12 px-8 rounded-xl bg-yellow-500 text-black font-black disabled:opacity-40 hover:bg-yellow-400 transition-colors">Seçili Planı Etkinleştir</button>
  </section>;
};
