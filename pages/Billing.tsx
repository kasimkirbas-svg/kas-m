...
export const Billing: React.FC<BillingProps> = ({ user, onSelectPlan }) => {
  ...
  return <section className="max-w-6xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
    ...
    <div className="grid md:grid-cols-2 gap-5">
      {plans.map(plan => <button key={plan.id} onClick={() => { setSelected(plan.id); setSuccess(false); }} className={`relative text-left p-6 sm:p-8 rounded-xl border transition-all ${selected === plan.id ? 'border-yellow-500 ring-2 ring-yellow-500/20 bg-yellow-50 dark:bg-yellow-500/5' : 'border-slate-200 dark:border-white/10 bg-white/75 dark:bg-[#101010]/80 hover:border-yellow-500/50'}`}>
