...
import { 
  ...
  Instagram, Facebook, Linkedin, Youtube, EyeOff, Eye
} from 'lucide-react';
...
const InputGroup = ({ icon: Icon, type, ...props }: any) => {
  ...
  return (
    <div className="relative group">
       ...
       <input 
          ...
          {...props}
          className={`w-full bg-[#050510] border border-slate-800 rounded-xl py-3.5 pl-11 ${isPassword ? 'pr-12' : 'pr-4'} text-white placeholder-slate-600 focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/30 transition-all duration-300 group-hover:border-slate-700 font-medium`}
       />
       {isPassword && (
          <button 
            type="button"
            onClick={() => setShowPassword(!showPassword)}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-amber-400 transition-colors duration-300 z-10 focus:outline-none"
          >
                              ...
                              >
                                  ...
                                  <div className="flex justify-between items-center px-1">
                                      <label className="flex items-center gap-2 text-xs text-slate-400 cursor-pointer hover:text-slate-300">
                                          <input type="checkbox" name="rememberMe" className="rounded border-slate-700 bg-slate-800 text-amber-500 focus:ring-amber-500/50" />
                                          Beni Hatırla
                                      </label>
                                  ...
                                  <div className="pt-2 flex flex-col gap-3">
                                    ...
                                    <button 
                                      ...
                                      className="w-full py-4 rounded-xl font-bold text-sm tracking-[0.1em] uppercase transition-all bg-slate-900 border border-slate-800 text-slate-300 hover:bg-slate-800 hover:text-white"
                                    >
                                        Kayıt Ol
                                    </button>
                                  </div>
