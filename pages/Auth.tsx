...
export default function Auth({ initialMode = "login", onAuthSuccess, onBack }: AuthProps) {
  ...
  return (
    <div className="light-auth min-h-screen bg-[#eef1f5] dark:bg-[#16222a] text-slate-900 dark:text-white font-sans relative overflow-x-hidden selection:bg-yellow-500/30">
      ...
      <header className="relative z-20 p-3 sm:p-7 flex items-center justify-between">
      ...
      <main className="relative z-10 w-full max-w-3xl mx-auto px-3 sm:px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-1 sm:pt-3">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-white/85 dark:bg-[#172a33]/95 backdrop-blur-2xl border border-white dark:border-white/10 p-4 sm:p-9 rounded-xl sm:rounded-2xl shadow-[0_24px_70px_rgba(15,23,42,0.18)] dark:shadow-[0_0_60px_rgba(0,0,0,0.5)]">
          <div className="text-center mb-6 sm:mb-7"><h1 className="text-2xl sm:text-3xl font-black text-slate-950 dark:text-white">{isLogin ? "Sisteme Giriş" : "Hesap Oluştur"}</h1><p className="text-slate-500 dark:text-slate-400 text-sm mt-2">{isLogin ? "Kayıtlı hesabınızla güvenli şekilde devam edin." : "Faturalama ve kullanıcı yönetimi için bilgilerinizi eksiksiz girin."}</p></div>
