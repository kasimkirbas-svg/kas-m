...
export const DocumentHistory: React.FC<{ onEdit?: (entry: HistoryEntry) => void }> = ({ onEdit }) => {
  ...
  return <section className="max-w-6xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
    ...
    <div className="relative mb-6"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-400" size={19} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Geçmişte ara..." className="w-full min-h-14 pl-12 pr-4 rounded-xl bg-white/85 dark:bg-[#171b20]/92 border border-slate-300 dark:border-white/10 text-slate-950 dark:text-white outline-none focus:border-amber-400 backdrop-blur-sm" /></div>
    {filtered.length ? <div className="space-y-3">{filtered.map(item => <article key={item.id} className="p-4 sm:p-5 rounded-xl bg-white/80 dark:bg-[#171b20]/92 border border-slate-200 dark:border-white/10 flex items-center gap-4 backdrop-blur-sm"><div className="w-11 h-11 rounded-lg bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 flex items-center justify-center shrink-0"><FileText size={21} /></div><div className="min-w-0 flex-1"><h2 className="font-bold text-slate-950 dark:text-white truncate">{item.title}</h2><p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{item.category} · {new Date(item.createdAt).toLocaleString('tr-TR')}</p></div>{item.formData && onEdit && <button onClick={() => onEdit(item)} title="Düzenlemeye devam et" className="w-10 h-10 rounded-lg flex items-center justify-center text-slate-400 hover:text-yellow-500 hover:bg-yellow-500/10"><FilePenLine size={17} /></button>}<button onClick={() => remove(item.id)} title="Kaydı sil" className="w-10 h-10 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-500/10"><Trash2 size={17} /></button></article>)}</div> : <div className="min-h-64 rounded-xl border border-dashed border-slate-300 dark:border-white/10 flex flex-col items-center justify-center text-center p-6"><FileClock size={44} className="text-slate-300 dark:text-slate-700" /><h2 className="mt-4 font-bold text-slate-800 dark:text-white">Henüz doküman geçmişi yok</h2><p className="mt-2 text-sm text-slate-500">Düzenleyiciden indirdiğiniz belgeler burada listelenecek.</p></div>}
  </section>;
};
