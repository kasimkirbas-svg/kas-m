...
export const Layout: React.FC<LayoutProps> = ({ children, user, currentView, onNavigate, onLogout }) => {
  ...
  return (
    <div className="light-app-shell min-h-screen overflow-x-hidden bg-[#16222a] text-[#f7f7f5] font-sans selection:bg-[#e5b82c]/30 relative">
      ...
      <main className="relative z-10 min-h-screen pt-16 sm:pt-[72px]">{children}</main>

      <button onClick={() => { setSupportOpen(true); setSupportState('idle'); }} className="fixed z-40 bottom-[max(0.75rem,env(safe-area-inset-bottom))] right-3 sm:bottom-6 sm:right-6 w-11 h-11 sm:w-auto sm:h-12 sm:px-5 rounded-full bg-[#111b22]/95 border border-amber-400/40 text-amber-300 shadow-xl backdrop-blur-xl flex items-center justify-center gap-2 text-sm font-bold"><HeadphonesIcon size={18} /><span className="hidden sm:inline">7/24 Canlı Destek</span></button>
