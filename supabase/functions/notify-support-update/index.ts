...
Deno.serve(async request => {
  ...
  try {
    ...
    const { data: profile, error: profileError } = await serviceClient.from('profiles').select('email,name').eq('id', ticket.user_id).single();
    if (profileError || !profile?.email) throw new Error('Kullanıcının e-posta adresi bulunamadı.');
    ...
    const mailResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from,
        to: [profile.email],
        subject: `Destek talebiniz yanıtlandı: ${ticket.subject}`,
